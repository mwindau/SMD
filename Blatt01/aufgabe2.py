...
# = alpha**2/s * (sin**2+2)*gamma**2/(gamma**2 * sin**2 + cos**2)
#c) Der Stabilitätsunterschied wird sichtbar wenn man sich die Maxima an den Polen
#anguckt.
#d) K = |theta * (2sin()cos()(-3*Beta**2+1))/((2+sin**2)*(1 - Beta**2 * cos**2)**2)|
#Problem ist schlecht für K >= 1 konditioniert. Im Graphen wird schlechte von guter
#Konditionierung durch die Linie unterschieden.


beta = np.sqrt(1 - ((511*1000)/(50*10**9))**2)
s = (2*50*10**9)**2
gamma = (50*10**9)/(511*1000)


def wirkungsqwinkelinstabil(x):
    return (const.alpha**2)/s * ((2 + np.sin(x)**2)/(1 - (beta**2) * np.cos(x)**2))

def wirkungsqwinkelstabil(x):
    return (const.alpha**2)/s * (2+np.sin(x)**2)*gamma**2/(gamma**2*np.sin(x)**2 + np.cos(x)**2)

def ableitungwirkungssqwinkelinstabil(x):
